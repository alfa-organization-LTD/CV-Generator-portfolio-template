...
const Footer: React.FC<FooterProps> = ({ contactUs }) => {
  return (
    <motion.footer
      className="flex flex-col items-center p-5 mt-10 rounded-t-2xl bg-[var(--border)] text-[var(--text)]"
      initial={{ opacity: 0, y: 40 }}
      whileInView={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.6, ease: 'easeOut' }}
      viewport={{ once: true }}
    >
      <h1 className="text-lg font-semibold">Follow me on:</h1>
      <div className="flex space-x-5 p-5">
        {/* GitHub Link */}
        <a href={contactUs?.github} aria-label="Zeiad's Github" target="_blank" rel="noopener noreferrer">
        ...
        {/* LinkedIn Link */}
        <a href={contactUs?.linkedin} aria-label="Zeiad's Linkedin" target="_blank" rel="noopener noreferrer">
      ...
      <p className="text-[var(--text-muted)]">CV Generator made with <span className='text-[var(--accent)]'>❤</span> by Ziad</p>
    </motion.footer>
  );
};
